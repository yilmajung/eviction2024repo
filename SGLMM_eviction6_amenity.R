...
saveRDS(fit_nonpayment, file='data/fit_nonpayment_final6_with_amenities.rds')

# Load the fitted model
fit_nonpayment <- readRDS("data/fit_nonpayment_final6_with_amenities.rds")
...
# Check R-hat and n-eff
max(summary(fit_nonpayment)$summary[,"Rhat"])
mean(summary(fit_nonpayment)$summary[,"Rhat"])
summary(fit_nonpayment)$summary[,"n_eff"]
min(summary(fit_nonpayment)$summary[,"n_eff"])
mean(summary(fit_nonpayment)$summary[,"n_eff"])
...
fire_police_pc <- posterior_estimates$beta_orig[,25]
food_pc <- posterior_estimates$beta_orig[,26]
college_univ_pc <- posterior_estimates$beta_orig[,27]
school_pc <- posterior_estimates$beta_orig[,28]
...
df_samples <- data_frame(intercept_samples, rent_mt50_samples, social_program_samples,
                         ...
                         fire_police_pc, food_pc, college_univ_pc, school_pc)
...
write.csv(df_95ci, "data/results/df_95ci_nonpayment_final_6_with_amenities.csv")
View(df_95ci)
################################
...
write.csv(df, 'data/df_geom_nonpayment_final_6_with_amenities.csv')
