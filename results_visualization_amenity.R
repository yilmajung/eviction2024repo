...
# Read in data
df_nonpay <- read_csv("data/df_95ci_nonpayment_final_6_with_amenities.csv")
head(df_nonpay)
colnames(df_nonpay) <- c("fixed", "lb_95", "ub_95", "beta_hat", "lb_90", "ub_90")

df_other <- read_csv("data/df_95ci_other_final_8_amenity.csv")
head(df_other)
...
df_nonpay$fixed <- c("intercept_samples", "rent >= 50%", "public assistance",
                     ...
                     'fire / police stations', 'cafe / restaurants', 'colleges', 'schools')
...
dim(df)
df$model <- c(rep("nonpayment", 30), rep("other", 30))
...
df %>%
  ...
  scale_x_continuous(limits = c(-0.42, 0.4)) +
...
pos_be <- position_nudge(y=c(rep(0.2, 8), rep(-0.2, 8)))
df %>%
  #filter(!str_detect(fixed, "time")) %>% # Remove time effects
  filter(fixed %ni% c("spatial_effects_samples", "intercept_samples", "multi-unit str")) %>%
